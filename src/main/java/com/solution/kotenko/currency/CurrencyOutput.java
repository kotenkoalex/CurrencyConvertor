...
public class CurrencyOutput extends CurrencyConvertor {
    ...
    static Logger LOGGER = Logger.getLogger(CurrencyOutput.class);
    ...
    public static void printAllCurrencyRates(String path, ArrayList<Currency> currencies) {
        try (FileWriter fileWriter = new FileWriter(path)) {
            for (Currency element : currencies) {
                ...
                LOGGER.info("Print all currency rates in today's file");
            }
        } catch (FileNotFoundException e) {
            /* will work when the file for today's date has already been created.
            therefore, it is not necessary to create and fill it anew */
            return;
        } catch (IOException e) {
            e.printStackTrace();
        }
